...
This should contain videos that have a translation of another language, where I speak both languages. The independent language (the one that is being translated) should have at least 90% completion. The dependent language (the language of the translation) should have a level of completion greater than 0 but less than 100.
...
This should contain any videos that have an original language that I speak that has not yet been subtitled completely.

h3. Section 4

This should contain any videos have a forked translation that I speak that has not been completed.

h3. Section 5

This should contain all other videos for the team. It is okay for now if it repeats videos from sections 1-4.

h2. How we've decided to achieve this

We achieve this goal by asking the user what languages they speak (if they have not already told us) and showing them four lists of videos, presented as one list.  (These lists correspond to the queries in "Team Detail Page Algorithm").
...
# Videos with incomplete translations in the user's languages.
# Videos with not yet started translations in the user's languages. (it is better for creating complete work that they help complete an existing work instead of starting a new one)
# Any incomplete original “direct from video” transcriptions in the user's language (a translator is more valuable than a transcriber because there will always be more English speakers than speakers of both English and French)
# All other videos, possibly allowing for duplication if there's something difficult about the query (I don't think this final category is reflected below).

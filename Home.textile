...
to show loading thing here.  *** Let's use the first spinner (from the first widget, or something similar) right on the metal button in the space to the left of "Done"
4. In Sync or Review, try double-clicking on sub. The elements overlap
a bit, so we need some design help here.
5. Legend keys not exhibiting mouseover behavior on FF3.5/Linux. I
spent a few minutes investigating this and it looks like there is no
obvious reason. Might be that :hover and :active pseudoclasses are
hokey. **** This isn't a big deal.

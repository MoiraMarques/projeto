...
# Make HTML for this same "early testing stages" warning message in widget.  Should appear when user clicks and opens dropdown menu.  It can be wider than the video if necessary or it can wrap (maybe centered?).  Just make something and try it with different video sizes to make sure it doesn't look totally f'ed.  See attached screenshot (search email for the text of this todo)
...
h3. Mini project: try to get widget down to 551 pixels (or as close as possible)
...
Next design iterations (just a note for Holmes):

Change to button-based (not key based)interface
# Send morgan mockups
# make ticket for adam on autopause dropdown
# WITH BUTTONS CHANGE: make "on deck" subtitle purple (or some other unused color) and make it that color in the list too).  Send adam a note to make a ticket if code change is necessary.
# Design (or describe) a nice fade-in for the on-deck subtitle.  Should happen pretty fast, like 1 second.    On-deck subtitle should be 2 seconds long, centered in the empty space to the right of the playhead.  Show us a demo, then send to adam to make ticket.

Change to side by side, short interface
